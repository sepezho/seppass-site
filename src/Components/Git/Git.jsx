                ...
                return (<div className={s.GitBg}>
                        <div className={s.Git}>
                                ...
                                <div className={s.CoreText}>
                                        <p>
                                                Seppass может подвязаться к вашему gitHub. Эту идею я подчерпнул у менеджера pass, о котором я писал выше. Бот подключается к gitHub, при помощи токена, который вы создаете сами. Вы можете полностью контролировать доступ бота к вашим репозиториям. После добавления оного, бот сможет работать с репозиторием.
                                        </p>
                                        <p>

                                                И тут начинается самое интересное, благодаря этой возможности seppass превосходит конкурентов. После подключения бота к вашей git-системе вы сможете выполнять основные git-комманды прямо через бота! Если вам необходимо сделать бэкап содержимого вашей папки на сторонний сервис, вам нужно будет создать пустой репозиторий на gitHub, а затем вызвать команду "/gitinit" в боте. И вуаля, копия всех данных уже лежит на gitHub, в новом репозитории! Так же есть и другие git-комманды, такие как "/gittoken" "/gitpush" "/gitpull" и "/gitclone". У последней есть одна интересная деталь: при клонировании уже существующей папки с паролями на новй аккаунт seppass, вы должны иметь в корне копируемой c gitHub папки файлик "git_private_key.asc", с экспортируемым gpg-ключом. Тогда вы сможете импортнуть gpg ключ, для корректной работы бота.
                                        </p>
                                        <p>
